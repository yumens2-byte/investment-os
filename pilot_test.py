...
def run_round2():
    ...
    t.check("시스템 버전 v1.14.0", SYSTEM_VERSION == "v1.14.0")
...
def print_final_report(r1: dict, r2: dict):
    print("\n" + "=" * 60)
    print("  파일럿 테스트 최종 결과 (v1.14.0 — 유료 ETF 상세전략 + 포지션사이징 리포트)")
    print("=" * 60)
    for r in [r1, r2]:
    ...
    if all_ok:
        ...
        print("  → v1.14.0 변경사항: paid_report_formatter 신규 + run_view 유료 채널 추가")
        print("  → Git 업로드 진행 가능")
    else:
        for r in [r1, r2]:
            if not r["success"]:
