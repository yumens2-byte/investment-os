...
def run_round2():
    ...
    t.check("시스템 버전 v1.9.0", SYSTEM_VERSION == "v1.9.0")
...
def print_final_report(r1: dict, r2: dict):
    print("\n" + "=" * 60)
    print("  파일럿 테스트 최종 결과 (v1.9.0 — 텔레그램 듀얼 채널 발행 추가)")
    print("=" * 60)
    for r in [r1, r2]:
    ...
    if all_ok:
        ...
        print("  → v1.9.0 변경사항: 텔레그램 무료/유료 채널 자동 발행 (session=full)")
        print("  → Git 업로드 진행 가능")
    else:
        for r in [r1, r2]:
            if not r["success"]:
...
def main():
    parser = argparse.ArgumentParser(description="Investment OS Pilot Test v1.9.0")
