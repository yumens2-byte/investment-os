...
# ── 실적 트윗 길이 정책 ──
# X Premium 한도(25,000) 이내에서 가독성·정보량 균형으로 1000자 채택
MAX_TWEET_CHARS = 1000          # 면책조항 포함 전체 한도
DISCLAIMER_TEXT = "\n\n⚠️ 투자 참고 정보, 투자 권유 아님"
DISCLAIMER_LEN  = len(DISCLAIMER_TEXT)
MAX_BODY_CHARS  = MAX_TWEET_CHARS - DISCLAIMER_LEN - 8  # 여유 8자
MAX_RETRY       = 2             # 한도 초과 시 재요청 횟수 (총 시도 3회)
...
- 반드시 {max_chars}자 이내 (한국어, 면책조항 포함 전체 길이 기준)
- EPS/매출 서프라이즈 여부 + 가이던스 방향 + 시간외 반응 + 주요 사업부별 성과 포함 가능
- 매수/매도 의견 절대 불포함
...
[재요청 #{attempt}] 직전 응답이 {prev_len}자로 한도({max_chars}자)를 초과했습니다.
반드시 {max_chars}자 이내로 더 압축해서 다시 작성하세요.
불필요한 수식어/부연설명을 제거하고 핵심 팩트(EPS/매출/가이던스/시간외)만 남기세요."""
...
def analyze_big_tech_earnings(tickers: list) -> list:
    ...
    logger.info(f"[StockAnalyzer] v{VERSION} 시작 (대상={tickers})")
...
def _generate_earnings_tweet(ticker: str, company: str, stock_data: dict) -> str | None:
    """
    Gemini로 실적 요약 트윗 생성 (1000자 한도 + 재요청 로직).

    설계:
    - AI에게 명시적으로 1000자 이내 요청
    - 응답이 한도 초과 시 이전 길이를 알려주며 재요청 (최대 2회)
    - 면책조항 없으면 추가
    - 최종 실패 시 None 반환 → 호출자가 fallback_tweet 사용
    """
    try:
        ...
        base_prompt = EARNINGS_PROMPT.format(
            company=company,
            ticker=ticker,
        ...
        prev_len = 0
        for attempt in range(MAX_RETRY + 1):  # 0,1,2 → 총 3회
            if attempt == 0:
                prompt = base_prompt
            else:
                prompt = base_prompt + RETRY_INSTRUCTION.format(
                    attempt=attempt,
                    prev_len=prev_len,
                    max_chars=MAX_TWEET_CHARS,
                )
            ...
            try:
                result = call(
                    prompt=prompt,
                    model="flash-lite",
                    max_tokens=1200,  # 한국어 1000자 ≈ 700~900 토큰 + 여유
                    temperature=0.3,
                )
            except Exception as e:
                logger.warning(
                    f"[StockAnalyzer] {ticker} Gemini 호출 예외 (시도 {attempt+1}/{MAX_RETRY+1}): {e}"
                )
                return None

            if not (result.get("success") and result.get("text")):
                logger.warning(
                    f"[StockAnalyzer] {ticker} Gemini 응답 비정상 (시도 {attempt+1}/{MAX_RETRY+1})"
                )
                return None
            ...
            if "투자 권유 아님" not in tweet:
                tweet = tweet.rstrip() + DISCLAIMER_TEXT

            tweet_len = len(tweet)

            if tweet_len <= MAX_TWEET_CHARS:
                logger.info(
                    f"[StockAnalyzer] {ticker} Gemini 트윗 생성 성공 "
                    f"({tweet_len}자, 시도 {attempt+1}/{MAX_RETRY+1})"
                )
                return tweet

            logger.warning(
                f"[StockAnalyzer] {ticker} 한도 초과 "
                f"({tweet_len}자 > {MAX_TWEET_CHARS}자, 시도 {attempt+1}/{MAX_RETRY+1})"
            )
            prev_len = tweet_len

        # 모든 재시도 실패
        logger.warning(
            f"[StockAnalyzer] {ticker} Gemini {MAX_RETRY+1}회 시도 모두 한도 초과 → fallback 사용"
        )
        return None

    except Exception as e:
        logger.warning(f"[StockAnalyzer] {ticker} Gemini 처리 중 예외: {e}")
        return None


def _fallback_tweet(ticker: str, company: str, stock_data: dict) -> str:
    """Gemini 실패 시 rule-based fallback 트윗 (rule-based 본문은 항상 200자 이내)"""
